...
# State normalization
state_bounds = np.array([2.4, 4.0, 0.21, 2.0])  # Tighter bounds
...
# Agent
class Agent:
    def __init__(self, state_size, action_size, buffer_size=100000, batch_size=128, gamma=0.995, lr=1e-4, tau=5e-3):
...
agent = Agent(state_size, action_size, lr=1e-4, tau=5e-3)
state, _ = env.reset()
for _ in range(1000):
    action = env.action_space.sample()
    next_state, reward, terminated, truncated, _ = env.step(action)
    done = terminated or truncated
    agent.memory.add(state / state_bounds, action, reward, next_state / state_bounds, done)
    state = next_state
    if done:
        state, _ = env.reset()
...
for episode in range(1, n_episodes + 1):
    ...
    for t in range(max_t):
        ...
        if done:
            if episode >= 400:  # Detailed logs for Episodes 400-500
                print(f"\nEpisode {episode}, Step {t + 1}:")
                print(f"  State: {state}")
                print(f"  Action: {action} ({['Left', 'Right'][action]})")
                print(f"  Reward: {reward:.3f}")
                print(f"  Total Reward (Steps): {total_reward:.0f}")
            break
    ...
    print(
        ...
        f"Avg50: {np.mean(scores_window):.1f}, "
        f"Epsilon: {epsilon:.2f}, "
        f"Loss: {loss:.4f}" if loss is not None else f"Loss: N/A"
    )

    # Print average and max score for each 50-episode block
    if episode % 50 == 0:
        block_avg = np.mean(episode_block_scores[-50:])
        block_max = np.max(episode_block_scores[-50:])
        print(f"Episodes {episode-49}–{episode}: Avg Score: {block_avg:.1f}, Max Score: {block_max:.0f}")

    if np.mean(scores_window) >= 400 and episode >= 50:
        print(f"✅ Solved in {episode} episodes! Avg score: {np.mean(scores_window):.1f}")
        torch.save(agent.qnetwork_local.state_dict(), 'cartpole_dqn.pth')
        break
